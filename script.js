...
const peter = {
  ...
  showSummary: function () {
    this.summary = `${this.firstName} ${
      this.lastName
    } has completed ${this.calTaskCount()} tasks. ${
      this.firstName
    }'s average score is  ${this.calAvgScore()}, ${
      this.firstName
    }'s highest score is ${this.calMaxScore()}, and ${
      this.firstName
    }'s lowest score is ${this.calMinScore()}. `;
    return this.summary;
  },
};
...
document.getElementById("submit").onclick = function () {
  ...
  if (temp == "")
    throw console.log("No entry. Please enter a value between -88 and 58");
  if (isNaN(temp))
    throw console.log("Invalid input. Please enter a value between -88 and 58");
  if (temp <= -89)
    throw console.log("Input too low. Please enter a value between -88 and 58");
  if (temp >= 59)
    throw console.log(
      "Input too high. Please enter a value between -88 and 58"
    );
  if (temp > -89 && temp < 59) {
